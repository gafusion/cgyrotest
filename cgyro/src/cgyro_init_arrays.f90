...
!$omp do reduction(+:res_loc)
  do iv=nv1,nv2
     ...
     ie = ie_v(iv)
     do ic=1,nc
        res_loc(ic,is,1) = res_loc(ic,is,1)+w_xi(ix)*w_e(ie)*jvec_c(1,ic,iv_loc)**2*res_weight(ix,ie)
...
!$omp parallel do private(iv_loc,is,ix,ie,ic)
  do iv=nv1,nv2
     ...
     do ic=1,nc
        ...
        upfac2(ic,iv_loc,1) = jvec_c(1,ic,iv_loc)*res_weight(ix,ie)/res_norm(ic,is,1)
        upfac1(ic,iv_loc,2) = w_e(ie)*w_xi(ix)*abs(xi(ix))*vel(ie)*jvec_c(1,ic,iv_loc)*xi(ix)*vel(ie)
        upfac2(ic,iv_loc,2) = jvec_c(1,ic,iv_loc)/res_norm(ic,is,2)*xi(ix)*vel(ie)*currenton
     enddo
  enddo
